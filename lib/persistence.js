...
Persistence.prototype.persistCachedDatabase = function (cb) {
  ...
  var allDocs = this.db.getAllData();
 ...
 */
Persistence.prototype.treatRawData = function (rawData, cb) {
  ...
  if (data.length > 0 && corruptItems / data.length > this.corruptAlertThreshold) {
    return cb(new Error("More than " + Math.floor(100 * this.corruptAlertThreshold) + "% of the data file is corrupt, the wrong beforeDeserialization hook may be used. Cautiously refusing to start NestDB to prevent dataloss"));
  }
