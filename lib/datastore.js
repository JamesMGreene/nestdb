 ...
 * @param {Function} options.dateGenerator Optional, if set, this function will be used for generating dates. It takes no arguments and should return a date.
...
function Datastore(options) {
  ...
  if (typeof options.idGenerator === 'function') {
	...
	if (typeof options.dateGenerator === 'function') {
		this._dateGenerator = options.dateGenerator;
	}
...
Datastore.prototype.prepareDocumentForInsertion = function (newDoc) {
  ...
  if (Array.isArray(newDoc)) {
    ...
    var now = this._dateGenerator ? this._dateGenerator() : new Date();
        ...
        for (i = 0; i < candidates.length; i += 1) {
          if (model.match(candidates[i], query) && (multi || numReplaced === 0)) {
            ...
            if (self.timestampData) {
              ...
              modifiedDoc.updatedAt = self._dateGenerator ? self._dateGenerator() : new Date();
            }
