 ...
 * @param {String} data
 */
storage.writeAsStream = function (filename, data) {
  var tempFilename = filename + '~'
    , stream = fs.createWriteStream(tempFilename, { encoding: 'utf8' })
    ;
  ...
  async.waterfall(
    ...
    , function (cb) {
        var dataStream = intoStream(data);
    ...
    function (err) {
      // Delay the emission of this "error" event so that the consuming code can add an event listener first
