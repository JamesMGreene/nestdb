...
describe('Model', function () {
  describe('Serialization, deserialization', function () {
    ...
    it('undefined fields are removed when serialized', function () {
      ...
      var b = model.serialize(a)

      var c = model.deserialize(b)
    ...
    it('Reject field names beginning with a $ sign or containing a dot, except the four edge cases', function () {
      ...
      var a2 = { 'with.dot': 'totest' }

      var e1 = { $$date: 4321 }

      var e2 = { $$deleted: true }

      var e3 = { $$indexCreated: 'indexName' }

      var e4 = { $$indexRemoved: 'indexName' };
  ...
  describe('Deep copying', function () {
    it('Should be able to deep copy any serializable model', function () {
      ...
      var obj = { a: ['ee', 'ff', 42], date: d, subobj: { a: 'b', b: 'c' } }

      var res = model.deepCopy(obj)
  ...
  describe('Modifying documents', function () {
    it('Queries not containing any modifier just replace the document by the contents of the query but keep its _id', function () {
      ...
      var updateQuery = { replace: 'done', bloup: [ 1, 8 ] }
    ...
    it('Throw an error if trying to change the _id field in a copy-type modification', function () {
      ...
      var updateQuery = { replace: 'done', bloup: [ 1, 8 ], _id: 'donttryit' }
    ...
    it('Throw an error if trying to use modify in a mixed copy+modify way', function () {
      ...
      var updateQuery = { replace: 'me', $modify: 'metoo' }
    ...
    it('Throw an error if trying to use an inexistent modifier', function () {
      ...
      var updateQuery = { $set: { it: 'exists' }, $modify: 'not this one' }
    ...
    it('Throw an error if a modifier is used with a non-object argument', function () {
      ...
      var updateQuery = { $set: 'this exists' }
    ...
    describe('$set modifier', function () {
      it('Can change already set fields without modfifying the underlying object', function () {
        ...
        var updateQuery = { $set: { some: 'changed', nay: 'yes indeed' } }

        var modified = model.modify(obj, updateQuery)
      ...
      it('Can set sub-fields and create them if necessary', function () {
        ...
        var updateQuery = { $set: { 'yup.subfield': 'changed', 'yup.yop': 'yes indeed', 'totally.doesnt.exist': 'now it does' } }
      ...
      it("Doesn't replace a falsy field by an object when recursively following dot notation", function () {
        ...
        var updateQuery = { $set: { 'nested.now': 'it is' } }
    ...
    describe('$inc modifier', function () {
      it('Throw an error if you try to use it with a non-number or on a non number field', function () {
        (function () {
          ...
          var updateQuery = { $inc: { nay: 'notanumber' } }
    ...
    describe('$pull modifier', function () {
      ...
      it('Can use any kind of NestDB query with $pull', function () {
    ...
    describe('$max modifier', function () {
      ...
      it('Works on embedded documents', function () {
        ...
        var updateQuery = { $max: { 'somethingElse.number': 12 } }
    ...
    describe('$min modifier', function () {
      ...
      it('Works on embedded documents', function () {
        ...
        var updateQuery = { $min: { 'somethingElse.number': 8 } }
  ...
  describe('Comparing things', function () {
    ...
    it('Then dates', function () {
      ...
      var dates = [new Date(-123), new Date(), new Date(5555), new Date(0)]
  ...
  describe('Querying', function () {
    describe('Comparing things', function () {
      it('Two things of different types cannot be equal, two identical native things are equal', function () {
        ...
        var toTestAgainst = [null, 'somestring', 42, true, new Date(72998322), { hello: 'world' }] // Use another array so that we don't test pointer equality
        var i; var j
      ...
      it('Can test native types null undefined string number boolean date equality', function () {
        ...
        var toTestAgainst = [undefined, null, 'someotherstring', 5, false, new Date(111111), { hello: 'mars' }]
    ...
    describe('Getting a fields value in dot notation', function () {
      ...
      it('Can navigate inside arrays with dot notation, and return the array of values in that case', function () {
        ...
        dv = model.getDotValue({ nestdb: true, data: { planets: [ { name: 'Earth', number: 3 }, { name: 'Mars', number: 2 }, { name: 'Pluton', number: 9 } ] } }, 'data.planets.number')
        ...
        dv = model.getDotValue({ nestdb: true, data: { planets: [ { name: 'Earth', numbers: [ 1, 3 ] }, { name: 'Mars', numbers: [ 7 ] }, { name: 'Pluton', numbers: [ 9, 5, 1 ] } ] } }, 'data.planets.numbers')
      ...
      it('Can get a single value out of an array using its index', function () {
        ...
        dv = model.getDotValue({ nestdb: true, data: { planets: [ { name: 'Earth', number: 3 }, { name: 'Mars', number: 2 }, { name: 'Pluton', number: 9 } ] } }, 'data.planets.2')
        assert.deepEqual(dv, { name: 'Pluton', number: 9 })

        // Dot notation with index in the middle
        dv = model.getDotValue({ nestdb: true, data: { planets: [ { name: 'Earth', number: 3 }, { name: 'Mars', number: 2 }, { name: 'Pluton', number: 9 } ] } }, 'data.planets.0.name')
    ...
    describe('$lt', function () {
      ...
      it('Can compare strings, with or without dot notation', function () {
        model.match({ a: 'nestdb' }, { a: { $lt: 'nestdc' } }).should.equal(true)
        model.match({ a: 'nestdb' }, { a: { $lt: 'nestda' } }).should.equal(false)

        model.match({ a: { b: 'nestdb' } }, { 'a.b': { $lt: 'nestdc' } }).should.equal(true)
        model.match({ a: { b: 'nestdb' } }, { 'a.b': { $lt: 'nestda' } }).should.equal(false)
      })
    ...
    describe('Array fields', function () {
      it('Field equality', function () {
        ...
        model.match({ tags: ['node', 'js', 'db'], nestdb: true }, { tags: 'js', nestdb: true }).should.equal(true)
