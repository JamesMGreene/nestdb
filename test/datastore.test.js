...
var reloadTimeUpperBound = 80 // In ms, an upper bound for the reload time used to check createdAt and updatedAt
...
describe('Datastore', function () {
  ...
  it('Custom ID Generator', function (done) {
    ...
    var db = new Datastore({ idGenerator: function () { lastId = '' + Date.now(); return lastId } })
  ...
  describe('Insert', function () {
    ...
    it('If timestampData option is set, a createdAt field is added and persisted', function (done) {
      var newDoc = { hello: 'world' }; var beginning = Date.now()
    ...
    it("If timestampData is set but createdAt is specified by user, don't change it", function (done) {
      var newDoc = { hello: 'world', createdAt: new Date(234) }; var beginning = Date.now()
      d = new Datastore({ filename: testDb, timestampData: true, autoload: true })
      d.insert(newDoc, function (err, insertedDoc) {
        assert.isNull(err)
        Object.keys(insertedDoc).length.should.equal(4)
    ...
    it("If timestampData is set but updatedAt is specified by user, don't change it", function (done) {
      var newDoc = { hello: 'world', updatedAt: new Date(234) }; var beginning = Date.now()
      d = new Datastore({ filename: testDb, timestampData: true, autoload: true })
      d.insert(newDoc, function (err, insertedDoc) {
        assert.isNull(err)
        Object.keys(insertedDoc).length.should.equal(4)
            ...
            d.insert({ tf: 4, an: 'other' }, function (err, _doc3) {
              ...
              d.insert({ tf: 9 }, function (err, _doc4) {
                ...
                d.getCandidates({ r: 6, notf: { $in: [6, 9, 5] } }, function (err, data) {
                  ...
                  var doc3 = _.find(data, function (d) { return d._id === _doc3._id })

                  var doc4 = _.find(data, function (d) { return d._id === _doc4._id })
        ...
        function (cb) { // Test with query that doesn't match anything
          d.update({ somedata: 'nope' }, { newDoc: 'yes' }, { multi: true }, function (err, n) {
            ...
            d.find({}, function (err, docs) {
              ...
              var doc2 = _.find(docs, function (d) { return d.somedata === 'again' })

              var doc3 = _.find(docs, function (d) { return d.somedata === 'another' })
    ...
    it('Can update multiple documents matching the query', function (done) {
      ...
      function testPostUpdateState (cb) {
        d.find({}, function (err, docs) {
          ...
          var doc2 = _.find(docs, function (d) { return d._id === id2 })

          var doc3 = _.find(docs, function (d) { return d._id === id3 })
          ...
          d.insert({ a: 5, hello: 'pluton' }, function (err, doc3) {
            ...
            d.update({ a: 2 }, { $inc: { a: 10 } }, function (err, nr) {
              ...
              d.find({}, function (err, docs) {
                ...
                var d2 = _.find(docs, function (doc) { return doc._id === doc2._id })

                var d3 = _.find(docs, function (doc) { return doc._id === doc3._id })
                ...
                d.remove({ z: '2' }, {}, function () {
                  d.update({ z: '1' }, { $set: { 'yes': 'yep' } }, {}, function () {
                    ...
                    d.find({}, function (err, docs) {
                      ...
                      var doc1 = _.find(docs, function (doc) { return doc._id === newDoc1._id })
      ...
      it('ensureIndex can be called before a load and still be initialized and filled correctly', function (done) {
        ...
        fs.writeFile(testDb, rawData, 'utf8', function () {
          d.load(function () {
            ...
            var doc2 = _.find(d.getAllData(), function (doc) { return doc.z === '2' })

            var doc3 = _.find(d.getAllData(), function (doc) { return doc.z === '3' })
          ...
          d.insert({ a: 2, b: 20, c: 200 }, function (err, _doc2) {
            ...
            d.insert({ a: 3, b: 30, c: 300 }, function (err, _doc3) {
              ...
              d.update({ a: 2 }, { $inc: { a: 10, c: 1000 }, $set: { b: 30 } }, {}, function (err) {
                ...
                var doc3 = _.find(data, function (doc) { return doc._id === _doc3._id })
          ...
          [
            ...
            function (cb) {
              db.update({ foo: 'blah' }, { $set: { foo: 'blahze' } }, function (err) {
                ...
                doc2.foo.should.equal('blah')
                assert.isOk(doc2._id)
                var doc2Mod = records[2]
                assert.isOk(doc2Mod)
                doc2Mod.foo.should.equal('blahze')
                assert.isOk(doc2Mod._id)
                doc2Mod._id.should.equal(doc2._id)
                cb()
              })
            },
            function (cb) {
              db.remove({ foo: 'bar' }, function (err) {
                ...
                assert.isOk(doc2Mod._id)
                doc2Mod._id.should.equal(doc2._id)
                var doc1Del = records[3]
                assert.isOk(doc1Del)
                assert.notExists(doc1Del.foo)
                doc1Del.$$deleted.should.equal(true)
                assert.isOk(doc1Del._id)
                doc1Del._id.should.equal(doc1._id)
                cb()
              })
            },
            function (cb) {
          ...
          [
            ...
            function (cb) {
              db.update({ foo: 'blah' }, { $set: { foo: 'blahze' } }, function (err) {
                ...
                doc4.foo.should.equal('blah')
                assert.isOk(doc4._id)
                var doc4Mod = records[4]
                assert.isOk(doc4Mod)
                doc4Mod.foo.should.equal('blahze')
                assert.isOk(doc4Mod._id)
                doc4Mod._id.should.equal(doc4._id)
                cb()
              })
            },
            function (cb) {
              db.remove({ foo: 'bar' }, function (err) {
                ...
                assert.isOk(doc4Mod._id)
                doc4Mod._id.should.equal(doc4._id)
                var doc3Del = records[5]
                assert.isOk(doc3Del)
                assert.notExists(doc3Del.foo)
                doc3Del.$$deleted.should.equal(true)
                assert.isOk(doc3Del._id)
                doc3Del._id.should.equal(doc3._id)
                cb()
              })
            },
            function (cb) {
